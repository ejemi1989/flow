...
import { Puzzle, X, MoreVertical, Play, Copy, HelpCircle } from 'lucide-react';
...
const IntegrationNode = ({ data, id }: { data: { label: string; icon: string }; id: string }) => {
  ...
  return (
    <div className="group relative px-4 py-2 shadow-lg rounded-md bg-white border-2 border-indigo-500 min-w-[200px]">
      ...
      <div className="absolute -right-3 -top-3">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button 
              className="p-1.5 rounded-full bg-white border border-gray-200 shadow-sm hover:shadow-md transition-all duration-200 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
              onClick={(e) => {
                e.stopPropagation();
              }}
            >
